# Claude Code Project Instructions — v8-kotlin

## Project Overview

This is **v8-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`v8`](https://crates.io/crates/v8).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/v8/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/v8>
